...
def search_result():
    ...
    params = {'city': city}
    response = requests.post('http://localhost:5000/api/finduser', data=params).json()
...
def contact():
    ...
    params = {'to': email,
              'from': session['email']}
    response = requests.post('http://localhost:5000/api/createrequest', data=params).json()
    if response['created']:
...
def accept():
    ...
    params = {'from': email,
              'to': session['email'],
              'accepted': '1'}
    response = requests.post('http://localhost:5000/api/updaterequest', data=params).json()
    if response['updated']:
        # mail
        flash('The user was contacted')
...
def validate_register():
    ...
    response = requests.post("http://localhost:5000/api/createuser", data=params).json()
    if response['created']:
...
def validate_login():
    ...
    response = requests.post("http://localhost:5000/api/authenticate", data=params).json()
